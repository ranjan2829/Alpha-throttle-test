...
Measured **23 Aug 2026** on Origin. CI: none. 429s: **0**. [`reports/live-origin.json`](reports/live-origin.json)

| run | attempted | opened | merged | errors | 429 | wall |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| Proof | 8 | 8 | **8** | 0 | 0 | 8.1 s |
| Clean volume | 1 200 | 1 200 | **1 019** | 0 | 0 | 9.6 min |
| Live (in progress) | 5 200 | — | **4 805** / 100 000 | — | 0 | — |

| | / min |
| --- | ---: |
| open (clean 1 200) | **124.5** |
| merge (clean 1 200) | **105.7** |

Proof PRs [#56](https://cursor.com/codebase/allocations/Alpha-throttle-test/pull/56)–[#63](https://cursor.com/codebase/allocations/Alpha-throttle-test/pull/63). Author: Ranjan S `<ranjan@allocations.com>`.
...
```mermaid
flowchart TD
  G["user goal"] --> P["root planner<br/>writes plan.json  ·  no coding"]
  P --> W1["worker<br/>isolated context, one scoped task"]
  P --> W2["worker"]
  P --> V["verifier<br/>accept or reject"]
  P --> S["subplanner ↻<br/>same loop on a slice, depth + 1"]
  W1 --> H["handoffs/*.json"]
  W2 --> H
  V --> H
  S --> H
  V -->|reject| R["respawn target"]
  R --> W1
```

```mermaid
flowchart LR
  subgraph isolation["nodes/task-aN/"]
    T["task.json"]
    X["context.json"]
    O["handoff.json"]
  end
...
# Claude planner; keep chunking until 100k merges
...
- Not a promise of 500 PRs / second. The stats table is what Origin actually did.
