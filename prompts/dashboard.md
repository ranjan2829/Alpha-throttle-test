...
5. Publish the full UI-only snapshot to `ranjan-rgb/Recursive-Agent-Dashboard` `main`.
6. Repeat. After the original six gen-0 defects, open the next catalog repair. `--stop` is the only hard halt.
