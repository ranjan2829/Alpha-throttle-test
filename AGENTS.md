...
7. If you are improving the **self-improving dashboard**, run `npx tsx src/cli.ts dashboard-improve --generations 12 --publish`. That writes one unique file under `web/src/feed/` per generation and bumps generation. After the original 6 defects it opens the next unpublished quality-catalog repair instead of stopping. Pass `--pr` (or run with Origin logged in) so each generation opens one unique-file PR via the throttle git/adapter. `--publish` pushes the standalone Vite snapshot to `ranjan2829/alpha-throttle-dashboard` **main** after each heal (Vercel production branch `main`). Do not claim the UI lives on `ranjan-rgb` unless that transfer happened. Do not rewrite sibling widgets. Claude is the planner when `ANTHROPIC_API_KEY` is set; `--planner grok` is optional (XAI_API_KEY) and falls back.
