...
7. If you are improving the **self-improving dashboard**, run `npx tsx src/cli.ts dashboard-improve`. That writes one unique file under `web/src/feed/` and bumps generation. Do not rewrite sibling widgets. Claude is the planner when `ANTHROPIC_API_KEY` is set; `--planner grok` is optional (XAI_API_KEY) and falls back.
