...
export function applyDashboardImprovement(options: ApplyImprovementOptions): ApplyImprovementResult {
  ...
  const memory = existsSync(memoryPath) ? loadMemory(memoryPath) : emptyMemory();
  const defect = nextOpenDefect(memory);
  if (!defect) {
    throw new Error("dashboard memory has no open defects — gen 0 is fully repaired");
  }
...
export { defaultMemoryPath, emptyMemory, loadMemory, nextOpenDefect };
